...
from mlp_layer import CustomLinear, CustomMLP
from regularization import CustomBatchNorm1d
...
class GNN_layer(nn.Module):
    ...
    def __init__(self, in_feats: int, out_feats: int, e_types: list, drop_rate: float = 0.0,
        ...
        if self.bn_type in [2, 3]:  # Amazon dataset uses batch normalization type 2
            ...
            for e_t in self.e_types:
                self.edge_bn[e_t[1]] = CustomBatchNorm1d(out_feats)
        ...
        with g.local_scope():
            ...
            if self.bn_type in [2, 3]:
                if not self.multi_relation:
                    ...
                    g.edata['msg'] = self.edge_bn[self.e_types[0]](
                        g.edata['msg'], update_running_stats=update_bn)
                else:
                    ...
                    for e_t in g.canonical_etypes:  # iterate over all unique edge types
                        g.edata['msg'][e_t] = self.edge_bn[e_t[1]](
                            g.edata['msg'][e_t], update_running_stats=update_bn)
            ...
            for e_t in g.etypes:
                etype_dict[e_t] = (
                    ...
                    # which means the message passed from node v to node u is the `msg` field from the above `udf_edges` function
...
class GNN_backbone(nn.Module):
    ...
    def __init__(self, in_feats: int, hidden_feats: int, out_feats: int,
                 ...
                 mlp12_dim: int, mlp3_dim: int, bn_types: int):
